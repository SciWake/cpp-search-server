...
struct Document {
    Document() = default;
    Document(int id, 
             double relevance, int rating): id(id), 
             relevance(relevance), 
             rating(rating) {}
             int id = 0;
...
void FindTopDocuments(const SearchServer& search_server, const string& raw_query) {
    cout << "Результат поиска по запросу: "s << raw_query << endl;
    try {
...
void MatchDocuments(const SearchServer& search_server, const string& query) {
    try {
        cout << "Совпадение документов по запросу: "s << query << endl;
        ...
        cout << "Ошибка совпадения документов на запрос "s << query << ": "s << e.what() << endl;
    }
}
...
int main() {
    ...
    AddDocument(search_server, 3, "большой пушистый кот большой пес"s, DocumentStatus::ACTUAL, { 1, 2, 3, 4, 5 });

    // Поиск документов
    FindTopDocuments(search_server, "пушистый -пес"s);
    FindTopDocuments(search_server, "пушистый кот"s);
    FindTopDocuments(search_server, "большой пес"s);

    // Сопоставление документов с запросом
